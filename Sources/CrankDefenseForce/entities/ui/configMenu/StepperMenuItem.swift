...
class StepperMenuItem: ConfigMenuItem {
	static nonisolated(unsafe) let arrowsBitmapTable = try! Graphics.BitmapTable(
		path: "entities/ui/StepperMenuItem/arrows")
	...
	func updateArrowFocusAnimator() {
		if let arrowFocusAnimator = arrowFocusAnimator {
			...
			let alpha: Float =
				isFocused ? arrowFocusAnimator.currentPercent : 1 - arrowFocusAnimator.currentPercent
