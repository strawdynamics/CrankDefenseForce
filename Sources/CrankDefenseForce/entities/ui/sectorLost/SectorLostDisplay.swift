...
class SectorLostDisplay {
	...
	private static func buildRows(_ config: Config) -> [[SectorLostCell]] {
		...
		let totalEnemiesDestroyed =
			tracker.cpuRocketsDestroyed + tracker.cpuFastRocketsDestroyed + tracker.cpuSmallUfosDestroyed
			+ tracker.cpuBigUfosDestroyed
		...
		let totalPowerUpsCollected =
			tracker.pauseEnemiesPowerUpsCollected + tracker.repairBuildingPowerUpsCollected
			+ tracker.destroyEnemiesPowerUpsCollected
