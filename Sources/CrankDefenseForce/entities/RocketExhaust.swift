...
class RocketExhaust : BaseEntity {
	...
	nonisolated(unsafe) static let normalExhaustBitmapTable = try! Graphics.BitmapTable(path: "entities/RocketExhaust/normalExhaust")

	nonisolated(unsafe) static let bigExhaustBitmapTable = try! Graphics.BitmapTable(path: "entities/RocketExhaust/bigExhaust")
	...
	init(_ config: Config) {
		...
		sprite.image = type == .normal ? Self.normalExhaustBitmapTable[0] : Self.bigExhaustBitmapTable[0]
	...
	override func update() {
		...
		sprite.image = type == .normal ? Self.normalExhaustBitmapTable[frameIndex] : Self.bigExhaustBitmapTable[frameIndex]
