...
class PdxinfoReader {
	...
	func read() throws(PdxinfoReaderError) -> [String.UTF8View: String] {
		...
		let uint8Buffer = UnsafeBufferPointer<UInt8>(
			start: buffer.assumingMemoryBound(to: UInt8.self), count: bytesRead)
		let pdxinfoString = String(decoding: uint8Buffer, as: Unicode.UTF8.self)

		return parse(pdxinfoString: pdxinfoString)
	}

	private func parse(pdxinfoString: String) -> [String.UTF8View: String] {
		var parsed: [String.UTF8View: String] = [:]
