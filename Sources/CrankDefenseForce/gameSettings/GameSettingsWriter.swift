...
struct GameSettingsWriter {
	static func write() -> Bool {
		...
		let pointer = UnsafeMutableRawPointer.allocate(
			byteCount: MemoryLayout<File.FileHandle>.size,
			alignment: MemoryLayout<File.FileHandle>.alignment)
		...
		encoder.writeString(
			&encoder, GameSettings.timeOfDay.stringValue,
			Int32(GameSettings.timeOfDay.stringValue.utf8.count))

		encoder.addTableMember(&encoder, "controlScheme", 13)
		encoder.writeString(
			&encoder, GameSettings.controlScheme.stringValue,
			Int32(GameSettings.controlScheme.stringValue.utf8.count))

		encoder.addTableMember(&encoder, "debugMode", 9)
		encoder.writeString(
			&encoder, GameSettings.debugMode.stringValue,
			Int32(GameSettings.debugMode.stringValue.utf8.count))
	...
	static nonisolated(unsafe) var writeStringFunc:
		@convention(c) (UnsafeMutableRawPointer?, UnsafePointer<CChar>?, CInt) -> Void = {
			p, str, len in
			let file: File.FileHandle = p!.assumingMemoryBound(to: PlaydateKit.File.FileHandle.self)
				.pointee
