...
struct GameSettingsReader {
	...
	static nonisolated(unsafe) var decodeError:
		@convention(c) (UnsafeMutablePointer<json_decoder>?, UnsafePointer<CChar>?, Int32) -> Void = {
	...
	static nonisolated(unsafe) var didDecodeTableValue:
		@convention(c) (UnsafeMutablePointer<json_decoder>?, UnsafePointer<CChar>?, json_value) -> Void =
			{ decoder, key, value in
				if let key = key {
					...
					if valueType == json_value_type.string && keyString == "controlScheme".utf8 {
						GameSettings.controlScheme =
							ControlScheme.fromString(String(cString: value.data.stringval)) ?? .standard
						...
						GameSettings.debugMode =
							DebugMode.fromString(String(cString: value.data.stringval)) ?? .disabled
					} else if valueType == json_value_type.string && keyString == "timeOfDay".utf8 {
						GameSettings.timeOfDay =
